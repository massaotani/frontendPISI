...
const CreatePoint = () => {
  ...
  const [initialPosition, setInitialPosition] = useState<[number, number]>([
    -23.5572404,
    -46.6609831,
  ]); //tipo da position (que marcamos no mapa é um array de numeros)
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition((position) => {
      // posição inicial do usuário assim que ele abrir a aplicação
  ...
  function handleSelectedItem(id: number) {
    ...
    if (alreadySelected >= 0) {
      // REMOVE DO ARRAY (item fica branco de novo)
      ...
      // ADICIONA NO ARRAY (item fica verde)
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (selectedFile) {
      // se ele tiver enviado uma imagem
      data.append("image", selectedFile);
      console.log(selectedFile);
    }
    
    const retAux = await api.post("/points", data);
